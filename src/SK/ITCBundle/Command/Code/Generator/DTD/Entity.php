...
class Entity extends AbstractGenerator
{
	...
	protected function configure()
	{
		...
		$this->addArgument('entityParentClass', InputArgument::OPTIONAL, 'Entity Generalized Class', 
			'\\SK\\ITCBundle\\DTD\\Entity');
		...
		foreach ($items as $item)
		{
			...
			$properties = array(
				...
				(new PropertyGenerator("name", $item->getName()))->setDocBlock(
					new DocBlockGenerator(sprintf("%s Name", $classDescription), "", 
						array(
							new Tag("var", "string")
						))),
				
				(new PropertyGenerator("value", $item->getValue()))->setDocBlock(
					new DocBlockGenerator(sprintf("%s Value", $classDescription), "", 
						array(
							new Tag("var", "string")
						)))
			);
			$docblock = new DocBlockGenerator($classDescription, "", 
				array(
			...
			$fileGenerator->setClass(
				new ClassGenerator($name, $namespace, null, $parentClass, array(), $properties, array(), $docblock));
